...
PeriodSender::PeriodSender(int argc, char** argv, shared_ptr<EndPoint> endpoint_):
		endpoint(endpoint_), interval(3, 0), counter(0){
...
int PeriodSender::handle_timeout(const ACE_Time_Value &value, const void *pVoid) {
	...
	string str = endpoint->encode(message);
	if (stream.send_n(str.c_str(), str.size()) < 0) {
